...
def run_net1(network, inp):
    ...
    if len(inp) != network.input_size: raise Exception(f"Input size is {len(inp)}, network requires {input_size}.")
...
def simulate(network):
    return run_net1(network, np.random.choice([0,1], size=network.input_size))
